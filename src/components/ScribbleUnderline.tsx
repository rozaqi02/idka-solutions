export interface ScribbleUnderlineProps {
  variant?: 'wavy' | 'double' | 'zigzag' | 'arc'
  className?: string
}
...
export function ScribbleUnderline({ variant = 'wavy', className = '' }: ScribbleUnderlineProps) {
  ...
  switch (variant) {
    case 'wavy':
      return (
        <svg className={baseClass} viewBox="0 0 240 22" fill="none" aria-hidden="true">
          <path
    ...
    case 'zigzag':
      return (
        <svg className={baseClass} viewBox="0 0 240 22" fill="none" aria-hidden="true">
          <path
            d="M 5 15 Q 25 3, 45 15 T 85 15 T 125 15 T 165 15 T 205 15 T 235 10"
            stroke="currentColor"
            strokeWidth="3"
            strokeLinecap="round"
